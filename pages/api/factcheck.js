...
const REGIONS = {
  ...
  un: {
    label: "UN & ICC",
    who: "United Nations, ICC (International Criminal Court), ICJ, WHO, IMF, World Bank",
...
// Get today's cache key for a region
function cacheKey(region) {
  const today = new Date().toISOString().slice(0, 10);
  return `cache:${region}:${today}`;
}

// Try to get cached result from Upstash
...
// Save result to cache — expires after 24 hours
async function saveCache(region, result) {
  try {
    const url = process.env.KV_REST_API_URL;
    const token = process.env.KV_REST_API_TOKEN;
    if (!url || !token) return;
    ...
    // Set with 86400 second expiry (24 hours)
    await fetch(`${url}/set/${key}/${encoded}/ex/86400`, {
      headers: { Authorization: `Bearer ${token}` }
    });
  } catch(e) {
...
// Save to history
async function saveHistory(region, score, grade, summary) {
  try {
    const url = process.env.KV_REST_API_URL;
    const token = process.env.KV_REST_API_TOKEN;
    if (!url || !token) return;
    const today = new Date().toISOString().slice(0, 10);
    const payload = encodeURIComponent(JSON.stringify({ score, grade, summary, date: today, region }));
    await fetch(`${url}/lpush/score_history/${payload}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
export default async function handler(req, res) {
  ...
  // ── CHECK CACHE FIRST ──
  const cached = await getCached(region);
  if (cached) {
    console.log(`Cache HIT for ${region} — serving cached result`);
    return res.status(200).json({ ok: true, data: cached.data, region, regionLabel: R.label, regionWho: R.who, cached: true });
  }

  console.log(`Cache MISS for ${region} — running live fact-check`);
...
Fact-check 5-6 of the most interesting and checkable claims from the news below.
...
Return ONLY raw JSON — no markdown, no backticks:
{
  ...
  "claims": [{
    ...
    "confidenceScore": 4,
    "relatedClaims": [
      {"claim": "Brief description of a similar previous claim", "date": "Full date e.g. Saturday, 18 January 2026", "verdict": "FALSE"}
    ]
  }]
}
...
relatedClaims = array of up to 2 similar past claims, or empty array`
      }]
    });
    ...
    await saveCache(region, { data: parsed });
    await saveHistory(region, parsed.dailyScore, parsed.dailyGrade, parsed.summary);

    return res.status(200).json({ ok: true, data: parsed, region, regionLabel: R.label, regionWho: R.who, cached: false });
